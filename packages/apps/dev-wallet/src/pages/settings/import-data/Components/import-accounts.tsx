...
interface IImportedAccount {
  networkUUID: `${string}-${string}-${string}-${string}-${string}` | undefined;
  networkId: string;
      ...
      for (const [index, account] of accounts.entries()) {
        ...
        if (chainData.length === 1) {
          if (
            ...
            if (isKeysetGuard(chainData[0].guard)) {
              account.verify = 'verified';
              account.guard = chainData[0].guard as IKeysetGuard;
              account.overallBalance = chainData[0].overallBalance;
              account.chains = chainData[0].chains;
            } else {
              account.verify = 'not-keyset';
            }
          } else {
        ...
        if (chainData.length > 1) {
          ...
          if (correct) {
            if (isKeysetGuard(correct.guard)) {
              account.verify = 'verified';
