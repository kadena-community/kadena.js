...
export const formatGuardInput = (guardInput: {
  uriGuard: any;
  burnGuard: any;
  mintGuard: any;
  saleGuard: any;
  ...
  const filteredGuardInput = Object.fromEntries(
    Object.entries(guardInput).filter(([_, value]) => value !== '[EXCLUDED]'),
  );

  return {
    ...(filteredGuardInput.uriGuard && {
      uriGuard: filteredGuardInput.uriGuard,
    }),
    ...(filteredGuardInput.burnGuard && {
      burnGuard: filteredGuardInput.burnGuard,
    }),
    ...(filteredGuardInput.mintGuard && {
      mintGuard: filteredGuardInput.mintGuard,
    }),
    ...(filteredGuardInput.saleGuard && {
      saleGuard: filteredGuardInput.saleGuard,
    }),
    ...(filteredGuardInput.transferGuard && {
      transferGuard: filteredGuardInput.transferGuard,
    }),
  };
};
...
export const formatRoyaltyInput = (royaltyInput: {
  royaltyFungible: string;
...
export const generateSpireKeyGasCapability = (
  account: string,
...
export const checkConcretePolicies = (
  policies: Policy[],
): ICreateTokenPolicyConfig => {
