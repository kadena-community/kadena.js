...
export const DutchAuction = ({ tokenImageUrl, sale }: DutchAuctionProps) => {
      ...
      ? getCurrentPrice(
          {
            startDate:
              Number(
                (auctionDetails?.['start-date'] as unknown as IPactInt).int,
              ) * 1000,
            endDate:
              Number(
                (auctionDetails?.['end-date'] as unknown as IPactInt).int,
              ) * 1000,
  ...
  useEffect(() => {
    async function fetch() {
      ...
      const priceIntervalSeconds =
        Number(
          (auctionDetails?.['price-interval-seconds'] as unknown as IPactInt)
            .int,
        ) * 1000;
      setNextPriceChange(
        getTimeUntilNextPriceChange(priceIntervalSeconds, new Date().getTime()),
      );
    }
  ...
  useEffect(() => {
    ...
    const priceIntervalSeconds =
      Number(
        (auctionDetails?.['price-interval-seconds'] as unknown as IPactInt).int,
      ) * 1000;

    const currentPriceInterval = setInterval(
      () => updateCurrentPrice(auctionDetails),
  ...
  return (
    <div className={styles.twoColumnRow}>
      ...
      <div className={styles.tokenInfoClass}>
        ...
        {isActive && <span>Current Price: {currentPrice}</span>}
