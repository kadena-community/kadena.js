...
export const sendVerificationMail = async ({
  ...
  const link = `${Object.entries(organisation.domains)[0][1].value}/verify-email?oobCode=${oobCode}`;
  ...
  try {
    const data = await mg.messages.create(
      `sandbox82751cf702b4492eb83d87863e6ad350.mailgun.org`,
      {
        ...
        'v:organisationSendEmail': organisation.sendEmail,
        'v:userId': user.uid,
        'v:userEmail': user.email,
        'v:userDisplayName': user.displayName || '',
      },
    );
