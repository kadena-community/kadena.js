...
There are capabilities like `TRANSFER` and `XTRANSFER`, which work similarly to
the transfer feature on your online banking application. They help move tokens
between accounts. There are also capabilities for updates and checks, such as
`SUPPLY`, `TOKEN`, `ACCOUNT_GUARD`, and `RECONCILE`.
...
respectively. Lastly, `LEDGER` provides a security check, ensuring executed
...
Helper functions like `precision`, `enforce-unit`, `debit`, `credit`,
`get-token-info`, `TRANSFER-mgr`, `account-guard`, `ledger-guard`,
`enforce-token-reserved`, `truncate`, `enforce-transfer-policy`,
`credit-account`, `update-supply`, `key`, `sale-active`, `sale-account`, and
`transfer-crosschain` provide support to the core functions and capabilities.
