...
Some more specific capabilities include `ADJUST_POLICY`, which allows changes to
token-associated policies, and `ROTATE` which permits a change in the guard of
an account. `DEBIT` and `CREDIT` give the power to use debit or credit
functions, and `UPDATE_SUPPLY` allows adjustments to a token's total
circulation.
...
`offer`, `withdraw`, `buy`, `transfer`, `get-balance`, `details`, `rotate`,
`total-supply`, `get-uri`, and `adjust-policy` offer a variety of operations
from creating new tokens and accounts to managing token trading in the
marketplace.
