...
Interfaces represent an abstract api that a module may implement by issuing an
`implements` statement within the module declaration. Interfaces may import
definitions from other modules by issuing a [use](#use) declaration, which may
be used to construct new constant definitions, or make use of types defined in
the imported module. Unlike Modules, Interface versioning is not supported.
However, modules may implement multiple interfaces.
...
A guard is essentially a predicate function over some environment that enables a
...
Requiring capabilities allow for "private" or "restricted" functions than cannot
...
Pact guards turn pact executions into autonomous processes that can own assets,
...
Pact 3.7 gains a form of _genericism_ with _module references_. This is
motivated by the desire to interoperate between modules that implement a common
interface, and to be able to treat the indicated module as a data value to gain
...
Module references can be used as normal pact values, which includes storage in
the database.
