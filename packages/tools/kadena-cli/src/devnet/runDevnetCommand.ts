import { defaultDevnet, devnetDefaults } from '../constants/devnets.js';
import { getExistingDevnets } from '../utils/helpers.js';

import type { TDevnetsCreateOptions } from './devnetsCreateQuestions.js';
import { defaultDevnetIsConfigured, loadDevnet, writeDevnets } from './devnetsHelpers.js';
  ...
  program
    ...
    .action(async (args: TDevnetsCreateOptions) => {
      ...
      try {
        ...
        if (name == defaultDevnet && !defaultDevnetIsConfigured()) {
          writeDevnets(devnetDefaults[defaultDevnet]);
        }
