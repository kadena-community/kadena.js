...
export function getDevnetConfiguration(name: string): TDevnetsCreateOptions | null {
  const devnetFilePath = path.join(defaultDevnetsPath, `${name}.yaml`);
  ...
  return yaml.load(readFileSync(devnetFilePath, 'utf8')) as TDevnetsCreateOptions;
}
