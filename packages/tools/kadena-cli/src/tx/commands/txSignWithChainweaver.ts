 ...
 */
export const createSignTransactionWithChainweaver: (
  program: Command,
  version: string,
) => void = createCommand(
  'sign-with-chainweaver',
  'Sign a transaction using your external Chainweaver wallet',
  [
  ...
  async (config) => {
    try {
      debug('sign-transaction:chainweaver:action')({ config });
      const {
