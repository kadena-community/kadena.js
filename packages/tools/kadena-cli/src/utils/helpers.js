 ...
 */
export function* questionGenerator(args, questions) {
 ...
 */
export async function collectResponses(args, questions, config) {
    const responses = {
        ...args,
    };
    ...
    while (result.done === false) {
        ...
        responses[question.key] = await question.prompt(currentConfig, responses, args);
 ...
 */
export function createSimpleSubCommand(name, description, actionFn, options = []) {
    return (program) => {
...
export function getExistingNetworks() {
...
export function getExistingProjects() {
    ...
    try {
        return readdirSync(projectRootPath)
            ...
            return (filename.toLowerCase().startsWith(projectPrefix) &&
 ...
 */
export async function processProject(projectName, keys) {
    ...
    for (const key of keys) {
        if (key in combinedConfig) {
            filteredConfig[key] = combinedConfig[key];
        }
    }
 ...
 */
export function getQuestionKeys(arr) {
    return arr.map((question) => question.key);
}
/**
 ...
 */
export function clearCLI(full = false) {
