...
const isElement = (n: Cheerio<AnyNode> | null): n is Cheerio<Element> =>
  n !== null && n[0] && n[0].type === 'tag';
...
export const parser: DocumentParser = (html, maxLength) => {
  ...
  const body = defaultContainerSelector
    .split(', ')
    .reduce((a, s) => a ?? $(s), null as Cheerio<AnyNode> | null);
  ...
  function processSection(
    ...
    defaultSectionHeaderTagName: string,
  ): void {
    let content = '';
    let currentElement = element.next();
    while (
      currentElement.length > 0 &&
      currentElement[0].tagName !== defaultSectionHeaderTagName
    ) {
  ...
  headers.each((index: number, element: any) => {
    const titleNode = defaultSectionHeaderSubSelector
      ? $(element).find(defaultSectionHeaderSubSelector)
      : $(element);
    const header = titleNode.text();

    if (
      defaultSectionFilter.some((filter) =>
        new RegExp(filter, 'i').test(header),
      )
    )
      return;
