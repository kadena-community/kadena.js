...
test('1 Initiator, 2 signers. all participants sign -> Should be able to mint the connection token @xs', async ({
  initiator,
  signer1,
  signer2,
  spirekeyApp,
  ...
  await test.step('Scan the QR code and create an account for all signers', async () => {
    await Promise.all([
      ...
      (async () => {
        await signer2.goto(shareUrl);

        const popupPromise = signer2.waitForEvent('popup');
        await proofOfusApp.loginToMintWithSigner(signer2);

        const signerPopup = await popupPromise;
        signer2Credential = await spirekeyApp.createSpireKeyAccountFor(
          signerPopup,
          true,
        );
      })(),
  ...
  await test.step('Sign with the Signer', async () => {
    await Promise.all([
      ...
      (async () => {
        ...
        await proofOfusApp.signProofWith(signer2);
        const signerPopup = await popupPromise;
        await spirekeyApp.signTransaction(signerPopup, signer2Credential);
      })(),
    ]);
  });
