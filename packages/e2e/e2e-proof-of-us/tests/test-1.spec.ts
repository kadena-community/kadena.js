...
test.only('1 Initiator, 10 signers. all participants sign -> Should be able to mint the connection token @s', async ({
  initiator,
  signer1,
  signer2,
  signer3,
  ...
  await test.step('wait for all siners to be in the list', async () => {
    await proofOfUs.countallSigners(initiator, 10);
  });
